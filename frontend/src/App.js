...
async function fetchQuestions(triviaParams, server_secret, amount) {
  console.log(triviaParams.type);
  const response = await fetch(
    `/api/trivia?max_articles=${amount}&trivia_type=${triviaParams.type}`,
    {
      headers: new Headers({
  ...
  if (quizStarted) {
    ...
    if (isFinished) {
      content = (
        <FinalScreen
          correctAnswers={
            questions.filter((question) => question.userAnswer === true).length
          }
